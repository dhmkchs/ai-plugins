...
ai-plugins/
├── .claude-plugin/
│   └── marketplace.json      ← 카탈로그 (my-plugins) · source: ./claude/plugins/work
└── claude/
    ├── plugins/
    │   └── work/
    │       ├── .claude-plugin/plugin.json   ← 플러그인 정의 (work, v0.5.0)
    │       ├── commands/                     ← 슬래시 커맨드 13종 (/work:*)
    │       └── skills/                       ← 스킬 13종
    ├── INSTALL.md
    └── README.md             ← 이 문서
```
...
| 항목 | 값 |
|---|---|
| 마켓플레이스 이름 | `my-plugins` |
| 플러그인 | `work@my-plugins` (v0.5.0) |
...
```bash
claude plugin list
# > work@my-plugins   Version: 0.5.0   Scope: user   Status: enabled
```
...
> 옮겼다면 `marketplace remove my-plugins` 후 새 경로로 다시 `add`.
...
> 이렇게 넣으면 네임스페이스(`work:`)가 없어져 `bug-hunt` 같은 이름으로 노출된다. 계속 쓸 거면 플러그인 방식이 낫다.
...
claude plugin marketplace remove my-plugins
```
