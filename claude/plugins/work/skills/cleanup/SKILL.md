...
티켓 번호: 인자 → 현재 브랜치 → `.work/plans/`에서 `status: pr-open`인 것들을 보여주고 선택.
...
저장소가 지저분해지는 게 걱정이면 `.work/plans/done/`으로 옮기는 것까지만 제안한다.
...
- worktree 없음
- 플랜: status=done, .work/plans/PROJ-123.md 유지
- Jira: In Review → Done ✓
