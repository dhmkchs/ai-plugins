...
description: >
  ...
  `plan` produced `.work/plans/<TICKET>.md`. Resumable: picks up at the first
...
티켓 번호: 인자 → 현재 브랜치명에서 추출 → `.work/plans/` 목록을 보여주고 선택 → 묻는다.

`.work/plans/<TICKET>.md`가 없으면 여기서 멈춘다.
...
플랜 파일은 `.work/plans/`에 티켓별로 있으므로 그대로 쓰면 된다.
