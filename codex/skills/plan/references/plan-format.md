...
- 체크박스 `- [ ]` → `- [x]`, `커밋:` 뒤에 해시 기록
- 요구사항 표의 상태 `☐` → `☑`. 그 `검증 방법`의 테스트가 **실제로 존재하고 통과할 때만** 찍는다
- 슬라이스의 `테스트:` 줄은 **그 슬라이스가 추가할 유닛 테스트 목록**이다. `start`가 이걸 보고 쓴다.
  실제로 쓴 테스트가 달라졌으면 이 줄을 실제에 맞춘다.
  테스트가 없는 슬라이스(설정·마이그레이션)는 `테스트: 없음 — <대체 검증>`으로 명시한다
