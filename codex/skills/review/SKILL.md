...
### AI 코드 슬롭 — 이름을 붙여야 대조할 수 있다

AI가 쓴 코드는 특정 실패 모드로 수렴한다. "군더더기를 줄여라" 같은 추상 원칙으로는 안 잡히고,
**이름이 붙은 패턴**이라야 diff를 그 이름으로 훑을 수 있다. 아래 여덟 개로 한 번 통과시킨다.

| 이름 | 증상 | 조치 |
|---|---|---|
| **1인용 래퍼** | 호출자가 하나뿐인 컴포넌트·훅·유틸을 새로 만들었다 | 인라인으로 되돌린다. 두 번째 호출자가 생길 때 뽑는다 |
| **삼키는 catch** | `catch {}` / `catch (e) { console.error(e) }` 뒤로 정상 흐름이 계속된다 | 다시 던지거나 사용자에게 보이는 상태로 바꾼다 |
| **재서술 주석** | `// 사용자를 가져온다` 위의 `getUser()` | 지운다. 주석은 코드가 말 못 하는 **왜**만 |
| **예언적 추상화** | 요구사항에 없는 옵션·플래그·전략 인터페이스·확장용 유니온 | 지운다. 지금 쓰이는 경로만 남긴다 |
| **방어 중복** | 타입이나 상위에서 이미 보장된 값에 또 null 체크·`?.` | 하나만 남긴다. 둘 다 있으면 어느 쪽이 계약인지 사라진다 |
| **복붙 미수정** | `result2`, `UserServiceCopy`, 앞 케이스와 같은 fixture를 쓰는 두 번째 테스트 | 이름과 데이터를 실제 케이스로 고친다 |
| **테스트 형해화** | 목 호출 횟수만 단정하고 반환값·상태를 안 본다 | 행위 단정으로 바꾼다 (`test`) |
| **설정 표류** | 이번 작업과 무관한 tsconfig·eslint·package.json 변경이 diff에 있다 | 분리하거나 되돌린다 |

이 표에 걸린 것은 **대부분 AUTO-FIX가 아니다.** "지운다"는 판단에는 맥락이 필요하다 —
찾아서 목록에 올리고, 지울지는 사용자가 정한다.

**「1인용 래퍼」와 「예언적 추상화」는 감으로 판정하지 않는다.** 삭제 테스트를 돌린다.

```bash
# 이 모듈을 지웠다고 상상한다 — 호출부를 센다
grep -rn "from ['\"].*<모듈명>" src/ --include=*.ts | grep -v "src/<모듈경로>/"
```

- 호출부가 **각자 같은 로직을 다시 써야 한다** → 값을 한다. 지적하지 않는다
- 호출부가 **다음 계층을 직접 부르면 그만이다** → 통과 계층. 지적한다
- 호출부가 **1곳뿐** → 인라인 후보. 지적한다

판정 근거(호출부 수와 그들이 무엇을 다시 해야 하는지)를 지적에 함께 쓴다.
어휘와 나머지 판정 테스트는 `refactor`가 정본이다.
