...
### 배경
로그인 직후 대시보드 진입 시 약 10% 확률로 500이 반환된다. 재시도하면 정상.
서버 로그에 NullPointerException (UserContext.getTenant). 2026-07-28 배포 이후 발생(그 전 로그에는 없음).
전체 로그인의 약 10%에 영향 — 새로고침으로 우회 가능하나 첫인상이 나쁨.

### 내용
UserContext 초기화 이전 접근으로 추정되는 근본 원인을 찾아 수정한다.
재현:
1. 로그아웃 상태에서 /login 진입
2. 정상 계정으로 로그인
3. 리다이렉트된 /dashboard 에서 네트워크 탭 확인
기대: 200   실제: 약 10% 확률로 500
범위 밖: 대시보드 성능 개선, UserContext 전반 리팩터링
...
비밀번호 재설정 메일이 잘못된 계정으로 간다. 현재 중복 계정 47건(2026-07-30 기준 쿼리), 고객문의 월 3~4건.

### 내용
가입 시 이메일 중복을 서버에서 검증해 거부한다. 대소문자 정규화와 동시 요청 처리를 포함한다.
범위 밖: 기존 중복 데이터 정리(→ 별도 티켓), 이메일 인증(verification) 플로우
...
왜 지금: 다음 스프린트의 주문 분할 기능이 이 경계를 그대로 확장해야 한다 —
지금 정리하지 않으면 같은 문제가 세 곳으로 늘어난다.

### 내용
@Transactional을 서비스 계층으로 내려 트랜잭션 경계를 좁힌다. 동작은 바꾸지 않는다.
범위 밖: 쿼리 최적화, 도메인 모델 변경
...
### 배경
14:02~14:14 결제 API 5xx. 원인: 커넥션 풀 고갈. 직접 원인은 배치 작업이 커넥션을 반환하지 않은 것.
임시 조치(완료): 배치 중단 후 애플리케이션 재시작, 14:14 복구.

### 내용
재발 방지 조치를 수행한다 — 커넥션 반환 누락 수정, 사용률 알람, 유사 패턴 점검.
범위 밖: 결제 API 전반의 회복탄력성 개선(→ 별도 에픽)
